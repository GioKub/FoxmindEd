...
const UserRepository = require('../db/repostory/user-repostory');
...
class UserService {
  ...
  loginService = async (nickname, password, salt) => {
    const { hashedPass } = await crypto.hashPassword(password, salt);
    return await this.UserRepository.loginDB(nickname, hashedPass);
  };
}
